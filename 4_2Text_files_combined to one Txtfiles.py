#2Text_files_combined to one Txtfiles (No Table Abstracts)
...
general_instruction = '''
- You have extracted information from the same abstract using two different instructions, resulting in the creation of two distinct TXT files.
- Each file contains a mixture of overlapping and non-overlapping columns, accompanied by their corresponding values.
- Your primary objective is to compose a new TXT file that retains shared information across the files and incorporates non-overlapping columns and their respective values from all two files.
...
- Ensure that shared information across the files is retained and incorporate non-overlapping columns and their respective values from all two files.
'''
...
for filename in os.listdir(folder1_path):
    if filename.endswith(".txt"):
        # Step 6: Read the content from the text files
        file_paths = [
            os.path.join(folder_path, filename) for folder_path in [folder1_path, folder2_path]
        ]
